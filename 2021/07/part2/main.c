...
result cost(int *position, int lowerBound, int upperBound)
{
    ...
    {
        // arithmetic series with difference one (kleiner Gauss)
        fuelCost += abs(guess - position[i]) * (abs(guess - position[i]) + 1) / 2;
        fuelCostUpOne += abs((guess + 1) - position[i]) * (abs((guess + 1) - position[i]) + 1) / 2;
        fuelCostDownOne += abs((guess - 1) - position[i]) * (abs((guess - 1) - position[i]) + 1) / 2;
    }
