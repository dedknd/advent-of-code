...
game bingoCheck(entry *board_array, int length, int draw);
...
int main(void)
{
    ...
    for (int i = 0; i < drawCount; i++)
    {
        ...
        game result = bingoCheck(board_array, length, draw_number[i]);
        ...
        if (board_array[i].value == draw)
        {
            ...
            for (int j = 0; j < BOARD_ROWS; j++)
            {
                ...
                if (colChecksum == 5)
                {
                    ...
                    game result = {TRUE, score, winnerBoard};
                    return result;
                }
            }
